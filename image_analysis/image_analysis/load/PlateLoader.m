...
% Modified Aug, 2018 & Feb 12, 2019 Erica Silva, 
%
    ...
    methods
        ...
        function img = load_image(this, filename)
            ...
            if isa(img, 'uint16')
                ...
                % img = single(img); # commented by ES 190212 
                img = im2double(img);
            end
            
            % added by ES to account for uint8 images
            if isa(img, 'uint8')
                % some toolkit functions require single or double
                img = im2double(img); 
            end            
            
            % added to account for images with alpha layer (Erica Silva)
            if size(img,3) > 3
