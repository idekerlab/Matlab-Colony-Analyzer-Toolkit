...
% Modified Erica Silva, February 2019
%
% Syntax
...
classdef ManualGrid < Closure
    properties
        ...
        gridspacing;
        numbercorners;
        nummiddleadjusts; % Added by Erica Silva
        numfulladjusts; % Added by Erica Silva
    ...
    methods
        function this = ManualGrid( varargin )
            ...
            this = default_param( this, ...
                ...
                'numberCorners', true, ...
                'numFullAdjusts', 1, ...
                'numMiddleAdjusts', 1,...% Added by Erica Silva
                varargin{:});
        end
    ...
    methods( Access = protected )
        ...
        function grid = manually_fit_grid(this, plate, grid)
            ...
            while (~success)
                ...
                % Adjust grid % Modified by Erica Silva
                if ( this.adjustgrid )
                    grid = adjust_grid( plate, grid, ...
                        'numMiddleAdjusts', this.nummiddleadjusts,...
                        'numFullAdjusts', this.numfulladjusts ); % modifiable see above
                end

                % Verify grid
                fig = figure('units','normalized','outerposition',[0 0 1 1]); % added by ES
% figure('position', [2000 0 1000 700]);
                movegui(fig, 'center');
                imagesc(plate)
                %colormap jet;
                %caxis([0 max(max(plate))/2]);% Added by ES
                hold on; scatter( grid.c(:), grid.r(:), '.r' ); hold off;
                snapnow;
